...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                @if($s->hasPaidConsultationFee())
                    <span class="badge bg-success">Paid</span>
                @else
                    <a href="{{ route('payments.create', ['patient_id' => $s->patient_id, 'eac_id' => $s->eac_id, 'payment_type' => 'eac_consultation']) }}" class="btn btn-outline-primary btn-sm">
                        Record Fee
                    </a>
                @endif
            </td>
            <td>
                @if($s->completion_status == 'Pending')
                    <form method="POST" action="/eac/{{ $s->eac_id }}/complete" style="display:inline;">
                        @csrf
