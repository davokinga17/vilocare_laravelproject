    ...
    <div class="d-flex gap-2">
        @if($payment->status === 'pending')
            <form method="POST" action="{{ route('payments.accept_cash', $payment) }}" onsubmit="return confirm('Confirm that cash was received for this service?')">
                @csrf
                <button type="submit" class="btn btn-success">Accept Cash Payment</button>
            </form>
        @endif
        @if($payment->payment_method === 'mtn_momo')
            <form method="POST" action="{{ route('payments.gateway.refresh', $payment->payment_id) }}">
                @csrf
                <button type="submit" class="btn btn-outline-primary">Refresh Gateway Status</button>
            </form>
        @endif
        @if($payment->payment_method === 'mtn_momo' && in_array($payment->status, ['failed', 'cancelled'], true))
            <form method="POST" action="{{ route('payments.retry', $payment->payment_id) }}">
                @csrf
                <button type="submit" class="btn btn-outline-warning">Retry Payment</button>
            </form>
        @endif
...
<div class="card shadow-sm">
    <div class="card-body">
        <div class="row gy-3">
            ...
            <div class="col-md-6"><strong>Method:</strong> {{ $paymentMethodOptions[$payment->payment_method] ?? strtoupper(str_replace('_', ' ', $payment->payment_method)) }}</div>
            ...
            <div class="col-md-6"><strong>Accepted By:</strong> {{ $payment->acceptedBy?->name ?: 'Not yet accepted' }}</div>
            <div class="col-md-6"><strong>Accepted At:</strong> {{ optional($payment->accepted_at)->format('d M Y H:i') ?: 'Not yet accepted' }}</div>
            ...
            @if($payment->payment_method === 'mtn_momo')
                ...
                @endif
            @endif
            @if(data_get($payment->meta, $payment->payment_method . '.simulation'))
                <div class="col-12">
                    <div class="alert alert-info mb-0">
                        This payment was completed using the built-in localhost simulator for {{ $paymentMethodOptions[$payment->payment_method] ?? $payment->payment_method }}.
                    </div>
                </div>
            @endif
            @if(data_get($payment->meta, 'retried_from_payment_id'))
                <div class="col-12">
                    <div class="alert alert-info mb-0">
                        This payment was created as a retry of payment #{{ data_get($payment->meta, 'retried_from_payment_id') }}.
                    </div>
                </div>
            @endif
            @if($payment->notes)
